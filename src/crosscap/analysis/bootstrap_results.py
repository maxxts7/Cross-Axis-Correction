...
Reproduces the analysis in data/runs/qwen_full/.../bootstrap.txt for any
results directory. Auto-discovers the four reclassified CSVs (with or
without the Windows-style ' (N)' suffix), computes label rates among
judgeable rows, builds 95% percentile CIs from N=10,000 prompt-level
resamples, and emits a paired lift between cross-cap and assistant-cap
on the shared jailbreak set.

Usage:
  python -m crosscap.analysis.bootstrap_results data/runs/llama_full
  python -m crosscap.analysis.bootstrap_results data/runs/llama_full --n-resamples 20000
  python -m crosscap.analysis.bootstrap_results data/runs/llama_full --label-col llm_label_opus
...
Outputs:
  <dir>/bootstrap.txt  -- text-format summary, same layout as the
                          existing data/runs/qwen_full bootstrap.txt.
"""
