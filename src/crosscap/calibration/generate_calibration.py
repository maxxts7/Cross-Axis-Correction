...
classify_calibration module handles the labelling step afterwards.

Output: <output-dir>/raw_generations.csv with columns
    prompt_idx, source, prompt_text, baseline_text

Usage:
    python -m crosscap.calibration.generate_calibration \\
        --source both \\
        --n-prompts 200 \\
...
as the main run_crosscap module, with no capping applied.
"""
...
from crosscap.core.crosscap_experiment import SteeringExperiment, generate_baseline
from crosscap.core.run_crosscap import (
    MODEL_NAME,
    AXIS_PATH,
    load_jbb_behaviors,
...
def main() -> None:
    ...
    print(f"Next: python -m crosscap.calibration.classify_calibration --input {raw_path} --output-dir {args.output_dir}")
