...
1. Build a fresh compliance axis (pca or mean_diff), save it, and run the sweep:
    ...
    python -m crosscap.sweeps.steer_probe --build --axis-method pca \\
        --output-dir steer_probe_pca

2. Reuse an existing axes.pt or warmup.pt:

    python -m crosscap.sweeps.steer_probe --axes-path "llama75/warmup (4).pt" \\
...
from crosscap.core.crosscap_experiment import (
    SteeringExperiment,
    compute_pca_compliance_axis,
    compute_mean_diff_compliance_axis,
)
from crosscap.core import run_crosscap  # for data loaders + outcome-calibration helper
...
# Llama's calibration (which uses data/calibration/, not JBB); the WJ eval
...
def build_axes(exp: SteeringExperiment, args) -> dict:
    ...
    # data/calibration/, other models use JBB + WildJailbreak.
...
def main():
    ...
    parser.add_argument("--calibration-dir", default=None,
                        help="Outcome-labelled refusing.csv/compliant.csv directory. "
                             "Defaults to data/calibration/ for Llama.")
