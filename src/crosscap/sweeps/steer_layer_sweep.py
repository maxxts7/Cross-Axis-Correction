...
from crosscap.core import run_crosscap

from crosscap.sweeps.steer_probe import (
    _SteerToTargetHook,
    generate_baseline,
    decode_new_tokens,
...
def generate_steered_on_range(
    ...
    `scope` is forwarded to the hook -- 'all' clamps every forward pass,
...
def main():
    ...
    parser.add_argument(
        ...
        help="Where to apply the steering clamp. "
             "'all' (default) clamps every forward pass -- prefill cursor plus "
             "every generated token. "
             "'prefill_only' clamps only the post-instruction cursor (step 0). "
             "'first_token_only' clamps only the first decoded token (step 1). "
             "'cursor_plus_first' clamps steps 0 and 1 together.",
    )
