...
echo "=== Step 1: Warmup ==="
python -m crosscap.core.run_crosscap --preset "$PRESET" --warmup
echo ""
...
for i in $(seq 0 $((N_GPUS - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=$i python -m crosscap.core.run_crosscap --preset "$PRESET" --chunk "$i/$N_GPUS" &
    pids+=($!)
done
...
python -m crosscap.core.run_crosscap --preset "$PRESET" --merge
