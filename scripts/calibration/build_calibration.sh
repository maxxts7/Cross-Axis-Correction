...
# Resolve project root (this script lives at scripts/calibration/), then
# cd there so all relative paths resolve consistently.
cd "$(dirname "$0")/../.."
...
python -m crosscap.calibration.classify_calibration \
    --output-dir "$OUTPUT_DIR" \
    --backend "$BACKEND"
