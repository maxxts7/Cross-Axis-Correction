...
# there so all relative paths (data/, output dirs) resolve consistently.
cd "$(dirname "$0")/../.."
...
TARGETS="8,16"
CALIBRATION_DIR="${CALIBRATION_DIR:-data/calibration}"
...
if [ "$MODE" = "pca" ] || [ "$MODE" = "mean_diff" ]; then
    ...
    echo "============================================"
    echo ""
    python -m crosscap.sweeps.steer_layer_sweep \
        --build \
        --model "$MODEL" \
