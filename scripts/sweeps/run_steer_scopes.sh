...
# Resolve project root (this script lives at scripts/sweeps/), then cd
# there so all relative paths (output dirs etc.) resolve consistently.
cd "$(dirname "$0")/../.."
...
case "$VARIANT" in
    basic)   MODULE="crosscap.sweeps.steer_layer_sweep" ;;
    mixed)   MODULE="crosscap.sweeps.steer_layer_sweep_mixed" ;;
    jbb)     MODULE="crosscap.sweeps.steer_layer_sweep_jbb_compliance" ;;
